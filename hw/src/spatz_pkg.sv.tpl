...
% if cfg['mempool']:
  ...
  typedef struct packed {
    ...
    // side channel: spatz_mem_req_o passes through a spill register, so anything sampled
    // from a live VLSU register when the request reaches the tile belongs to a LATER
    // burst. Only meaningful when burst_len > 1.
