...
`ifdef MEMPOOL_SPATZ
      // An FP->int move (e.g. fmv.x.w) writes a scalar GPR; MemPool's Snitch
      // only retires an accelerator response when acc_pwrite=1, so flag it as
      // a write. Mirrors the controller's rsp_d.write=1. Without this the
      // move's result is never written back and a dependent instruction
      // deadlocks the core.
